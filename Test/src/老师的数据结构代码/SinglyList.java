...
{
    public Node<T> head,rear;//头指针，指向单链表的头结点
    public int n;

    //（1）构造方法(动态链表)
    public SinglyList()                                    //构造方法，构造空单链表
    {
        this.rear=this.head = new Node<T>();//创建头结点，data和next值均为null
        this.n=0;
    }
    ...
    {
        this();                                            //创建空单链表，只有头结点
        rear=this.head;                            		//rear指向单链表最后一个结点
...
//      String str="(";
    ...
    {
        int i=0;
        for (Node<T> p=this.head.next;  p!=null;  p=p.next) //p遍历循环单链表
            i++;
        return i;
    }
    ...
    public Node<T> insert2(Node<T> point, T x)
    {
        if (x==null)
            throw new NullPointerException("x==null");
        Node<T> front=this.head;
        while (front.next!=point)
            front = front.next;
